...
//				var socketAddressInet = UnsafePointer<sockaddr_in>(socketAddressPointer).pointee
                var socketAddressInet = UnsafePointer(socketAddressPointer).withMemoryRebound(to: sockaddr_in.self, capacity: 1) {
                    $0.pointee
                }
...
//				var socketAddressInet6 = UnsafePointer<sockaddr_in6>(socketAddressPointer).pointee
                var socketAddressInet6 = UnsafePointer(socketAddressPointer).withMemoryRebound(to: sockaddr_in6.self, capacity: 1) {
                    $0.pointee
                }
...
//		}
		...
		newResponseSource.setEventHandler() {
			...
			let bytesRead = withUnsafeMutablePointer(to: &socketAddress) {
                $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { serverAddr in
                    recvfrom(UDPSocket, UnsafeMutableRawPointer(mutating: response), response.count, 0, serverAddr, &socketAddressLength)
                }
			...
			guard let endpoint = withUnsafePointer(to: &socketAddress, {
                $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { serverAddr in
                    self.getEndpointFromSocketAddress(socketAddressPointer: serverAddr)
                }
...
//					sendto(UDPSocket, data.bytes, data.count, 0, UnsafePointer($0), socklen_t(serverAddress.sin.sin_len))
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { serverAddr in
                        data.withUnsafeBytes {
                            sendto(UDPSocket, $0, data.count, 0, serverAddr, socklen_t(serverAddress.sin.sin_len))
                        }
			...
			case AF_INET6:
				...
				sent = withUnsafePointer(to: &serverAddress.sin6) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { serverAddr in
                        data.withUnsafeBytes {
                            sendto(UDPSocket, $0, data.count, 0, serverAddr, socklen_t(serverAddress.sin6.sin6_len))
                        }
